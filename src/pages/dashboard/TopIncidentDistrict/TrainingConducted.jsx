...
import { getchartTrainingList } from "../../../services/DashboardCard";
...
export default function TrainingConducted() {
  ...
  const options = useMemo(() => ({
      ...
      tooltip: {
        ...
        callbacks: {
          label: function (context) {
            return `Training: ${context.parsed.y}`;
          },
    ...
    elements: {
      point: {
        radius: 4,
        hoverRadius: 6,
        backgroundColor: "#DF7914",
        borderColor: "#fff",
        borderWidth: 1,
        hoverBackgroundColor: "#DF7914",
  ...
  const mapApiDataToChartFormat = useCallback((apiData) => {
    // Handle the API typo: 'tainingCount' instead of 'trainingCount'
    const monthlyData = apiData.tainingCount || [];
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const response = await getchartTrainingList(selectedYear);
